...
import StoreModalContent from './Components/StoreModalContent'
...
class Store extends Component {
    ...
    serverSideDataSource = () => {
        ...
        return {
            getRows(params) {
                ...
                StoreController.workerListing(filters).then((response) => {
                    if (response.success) {
                        that.setState({
    ...
    profileImageUi = (id) => {
        const promise = new Promise((resolve, reject) => {
            ...
            StoreController.user_Details(id, data).then((response) => {
                if (response.success) {
    ...
    addWorker = () => {
        ...
        if (valid) {
            ...
            let type = 'post';
            loaderStateTrue();
            StoreController.workerAdd([dataset.user_details], type).then((response) => {
    ...
    workerRelationInsert = (data, dataset) => {
        const promise = new Promise((resolve, reject) => {
            ...
            loaderStateTrue();
            StoreController.userRelationalDetailsAdd([dataset], 'post').then((response) => {
                loaderStateFalse();
                if (response.length > 0) {
                    resolve(response)
                    response.map((data, index) => {
...
export default withRouter(connect(mapStateToProps, { loaderStateTrue, loaderStateFalse, setToken, setUserCredentials, logOutApp })(Store));