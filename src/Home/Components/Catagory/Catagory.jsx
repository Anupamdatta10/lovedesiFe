...
import CatagoryModalContent from './Components/CatagoryModalContent'
import { AgGridReact } from '@ag-grid-community/react';
import { InfiniteRowModelModule } from '@ag-grid-community/infinite-row-model';
import "ag-grid-community/styles/ag-grid.css";
import "ag-grid-community/styles/ag-theme-alpine.css";
import * as CatagoryController from './Controller/CatagoryController';
...
class Catagory extends Component {
    ...
    serverSideDataSource = () => {
        ...
        return {
            getRows(params) {
                ...
                CatagoryController.workerListing(filters).then((response) => {
                    if (response.success) {
                        that.setState({
    ...
    profileImageUi = (id) => {
        const promise = new Promise((resolve, reject) => {
            ...
            CatagoryController.user_Details(id, data).then((response) => {
                if (response.success) {
    ...
    addWorker = () => {
        ...
        if (valid) {
            ...
            let type = 'post';
            loaderStateTrue();
            CatagoryController.workerAdd([dataset.user_details], type).then((response) => {
    ...
    workerRelationInsert = (data, dataset) => {
        const promise = new Promise((resolve, reject) => {
            ...
            loaderStateTrue();
            CatagoryController.userRelationalDetailsAdd([dataset], 'post').then((response) => {
                loaderStateFalse();
                if (response.length > 0) {
                    resolve(response)
                    response.map((data, index) => {
...
export default withRouter(connect(mapStateToProps, { loaderStateTrue, loaderStateFalse, setToken, setUserCredentials, logOutApp })(Catagory));